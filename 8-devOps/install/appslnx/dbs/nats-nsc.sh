...
install_nsc() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/nats-io/nsc/releases/latest |
    grep browser_download_url | grep nsc-linux-amd64.zip | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing nsc (nats nkey/jwt based configurations)..."
