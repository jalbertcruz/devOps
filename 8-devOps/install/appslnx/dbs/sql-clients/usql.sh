...
install_usql() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/xo/usql/releases/latest |
    grep browser_download_url | grep linux-amd64.tar.bz2 |
    grep -v static |
    head -n 1)
  ...
  echo "📥 Installing usql (Universal command-line interface for SQL databases)..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf usql*linux-amd64.tar.bz2
