...
install_nats_top() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/nats-io/nats-top/releases/latest |
  ...
  tar -xvf nats-top*linux_amd64.tar.gz
