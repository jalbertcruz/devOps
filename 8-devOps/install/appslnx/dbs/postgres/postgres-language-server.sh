...
app_name=postgres_language_server
install_postgres_language_server() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/supabase-community/postgres-language-server/releases/latest |
    grep browser_download_url | grep x86_64-unknown-linux-gnu | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing postgres-language-server..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  mv postgres-language-server_x86_64-unknown-linux-gnu postgres-language-server
  chmod +x postgres-language-server
  mv postgres-language-server $DEST
