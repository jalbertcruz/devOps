...
app_name=nats_server
install_nats_server() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/nats-io/nats-server/releases/latest |
    grep browser_download_url | grep inux-amd64.tar.gz | grep -v RC | head -n 1 |
    cut -d '"' -f4)
  ...
  tar -xvf nats-server*inux-amd64.tar.gz
  mv nats-server*inux-amd64/nats-server $DEST
