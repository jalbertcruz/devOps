...
install_nats_cli() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/nats-io/natscli/releases/latest |
    grep browser_download_url | grep inux-amd64.zip | head -n 1 |
    cut -d '"' -f4)
  ...
  unzip nats*inux-amd64.zip
  mv nats*inux-amd64/nats $DEST
