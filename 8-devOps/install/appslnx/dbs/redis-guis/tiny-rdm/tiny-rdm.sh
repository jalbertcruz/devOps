...
install_tiny_rdm() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -s https://api.github.com/repos/tiny-craft/tiny-rdm/releases/latest |
    grep browser_download_url | grep linux_amd64.AppImage | head -n 1)
  ...
  echo "📥 Installing tiny-rdm (Redis GUI)..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -LO
  mv tiny-rdm*linux_amd64.AppImage a.AppImage
  chmod +x a.AppImage
