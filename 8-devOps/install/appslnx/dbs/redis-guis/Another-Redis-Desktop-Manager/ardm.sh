...
app_name=another_redis_desktop_manager
install_another_redis_desktop_manager() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url1=$(curl -s https://api.github.com/repos/qishibo/AnotherRedisDesktopManager/releases/latest |
    grep browser_download_url | grep x86_64.AppImage | grep linux | head -n 1)
  ...
  echo "📥 Installing AnotherRedisDesktopManager (Redis GUI)..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -LO
  mv Another-Redis-Desktop-Manager-linux*x86_64.AppImage a.AppImage
  chmod +x a.AppImage
