...
app_name=p3x_redis_ui
install_p3x_redis_ui() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/patrikx3/redis-ui/releases/latest |
    grep browser_download_url | grep AppImage | grep -v arm | head -n 1)
