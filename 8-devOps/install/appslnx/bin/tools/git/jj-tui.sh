...
install_jj_tui() {
  ...
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/faldor20/jj_tui/releases/latest |
    grep browser_download_url | grep jj_tui_linux.zip | head -n 1 |
    cut -d '"' -f4)
