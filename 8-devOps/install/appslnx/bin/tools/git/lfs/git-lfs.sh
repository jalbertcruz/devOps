...
install_git_lfs() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/git-lfs/git-lfs/releases/latest |
    grep browser_download_url | grep git-lfs-linux-amd64 | head -n 1 |
    cut -d '"' -f4)
  ...
  tar -xvf git-lfs-linux-amd64*.tar.gz
  maybe_copy_fish_completions_files
  mv git-lfs*/git-lfs $DEST
...
if (
  ...
  echo "Waiting for $WAITING_TIME seconds before the next script..."
  sleep $WAITING_TIME
fi
