...
install_jj_fzf_git() {
  ...
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/tim-janik/jj-fzf/releases/latest |
    grep browser_download_url | grep jj-fzf.sfx | head -n 1 |
    cut -d '"' -f4)
