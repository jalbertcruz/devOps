...
install_git_absorb() {
  ...
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/tummychow/git-absorb/releases/latest |
  ...
  tar -xvf git-absorb*x86_64-unknown-linux-musl.tar.gz
  maybe_copy_fish_completions_files
  maybe_copy_man_pages_files
  mv git-absorb*x86_64-unknown-linux-musl/git-absorb $DEST
