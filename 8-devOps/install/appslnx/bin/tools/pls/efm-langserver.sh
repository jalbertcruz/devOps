...
app_name=efm_langserver
install_efm_langserver() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/mattn/efm-langserver/releases/latest |
  ...
  echo "📥 Installing efm-langserver: General purpose Language Server..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf efm-langserver*inux_amd64.tar.gz
  maybe_copy_fish_completions_files
  mv efm-langserver*inux_amd64/efm-langserver $DEST
