...
install_babashka() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/babashka/babashka/releases/latest |
    grep browser_download_url | grep inux-amd64.tar.gz | grep -v .sha256 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing babashka: Native, fast starting Clojure interpreter for scripting..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf babashka*inux-amd64.tar.gz
