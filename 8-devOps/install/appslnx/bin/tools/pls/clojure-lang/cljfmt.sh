...
install_cljfmt() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/weavejester/cljfmt/releases/latest |
    grep browser_download_url | grep linux-amd64 | grep -v static | grep -v .sha256 | head -n 1 |
    cut -d '"' -f4)
  ...
  tar -xvf cljfmt*linux-amd64.tar.gz
  cp cljfmt $DEST
