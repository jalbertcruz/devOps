...
app_name=clojure_jet
install_clojure_jet() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/borkdude/jet/releases/latest |
  ...
  echo "📥 Installing jet: CLI to transform between JSON, EDN, YAML and Transit using Clojure..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf jet-0.7.27-linux-amd64.tar.gz
