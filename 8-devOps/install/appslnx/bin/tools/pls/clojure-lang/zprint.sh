...
app_name=clojure_zprint
install_clojure_zprint() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/kkinnear/zprint/releases/latest |
    grep browser_download_url | grep zprint-filter | head -n 1 |
    cut -d '"' -f4)
