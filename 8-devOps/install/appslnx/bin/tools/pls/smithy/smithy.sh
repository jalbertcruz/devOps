...
app_name=smithy_language_server
install_smithy_language_server() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/smithy-lang/smithy-language-server/releases/latest |
    grep browser_download_url | grep linux-x86_64.zip | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing smithy-language-server..."
