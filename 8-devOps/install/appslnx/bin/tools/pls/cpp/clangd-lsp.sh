...
install_clangd_lsp() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/clangd/clangd/releases/latest |
    grep browser_download_url | grep clangd-linux- | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing clangd_lsp: clangd language server (https://clangd.llvm.org/)..."
