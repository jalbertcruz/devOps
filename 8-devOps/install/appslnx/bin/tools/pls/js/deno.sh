...
install_deno() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/denoland/deno/releases/latest | grep browser_download_url | grep x86_64-unknown-linux-gnu.zip | head -n 1 |
    cut -d '"' -f4)
  ...
  unzip *.zip
  mv deno $DEST
  deno completions fish >"$FISH_COMPLETIONS_DIR/deno.fish"
  chmod +x "$FISH_COMPLETIONS_DIR/deno.fish"
