...
install_biome() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/biomejs/biome/releases/latest |
    grep browser_download_url | grep biome-linux-x64 | head -n 1 |
    cut -d '"' -f4)
