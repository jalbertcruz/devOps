...
install_bun() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/oven-sh/bun/releases/latest |
    grep browser_download_url | grep bun-linux-x64.zip | head -n 1 |
    cut -d '"' -f4)
