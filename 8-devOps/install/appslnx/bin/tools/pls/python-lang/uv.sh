...
install_uv() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/astral-sh/uv/releases/latest |
  ...
  mv uv-x86_64-unknown-linux-gnu/uv $DEST
  mv uv-x86_64-unknown-linux-gnu/uvx $DEST
