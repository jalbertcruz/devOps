...
install_ruff() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/astral-sh/ruff/releases/latest |
  ...
  tar -xvf ruff-x86_64-unknown-linux-gnu.tar.gz
  maybe_copy_fish_completions_files
  mv ruff-x86_64-unknown-linux-gnu/ruff $DEST
