...
install_hatch() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/pypa/hatch/releases/latest |
    grep browser_download_url | grep hatch-x86_64-unknown-linux-gnu.tar.gz | head -n 1)
  ...
  tar -xvf hatch-x86_64-unknown-linux-gnu.tar.gz
