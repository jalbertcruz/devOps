...
install_age() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/FiloSottile/age/releases/latest |
    grep browser_download_url | grep inux-amd64.tar.gz | head -n 1)
  ...
  echo "📥 Installing age: A simple, modern and secure encryption tool (and Go library) with small explicit keys..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf age*inux-amd64.tar.gz
  maybe_copy_fish_completions_files
  maybe_copy_man_pages_files
  mv age/age age/age-keygen $DEST
