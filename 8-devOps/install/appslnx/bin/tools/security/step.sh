...
install_step() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/smallstep/cli/releases/latest |
  ...
  echo "📥 Installing step: A zero trust swiss army knife for working with X509, OAuth, JWT, OATH OTP, etc...."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf step_linux*amd64.tar.gz
  maybe_copy_fish_completions_files
  mv step*/bin/step $DEST
