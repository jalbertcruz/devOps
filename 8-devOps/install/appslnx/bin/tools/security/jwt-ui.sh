...
install_jwt_ui() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/jwt-rs/jwt-ui/releases/latest |
    grep browser_download_url | grep jwtui-linux.tar.gz | head -n 1 |
    cut -d '"' -f4)
