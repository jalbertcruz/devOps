...
install_totp_cli() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/yitsushi/totp-cli/releases/latest |
  ...
  echo "📥 Installing totp-cli: Authy/Google Authenticator like TOTP CLI tool..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf totp-cli_Linux_x86_64.tar.gz
