...
install_jwt_cli() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/mike-engel/jwt-cli/releases/latest |
    grep browser_download_url | grep jwt-linux.tar.gz |
    cut -d '"' -f4)
