...
install_ghz() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/bojand/ghz/releases/latest |
    grep browser_download_url | grep inux-x86_64.tar.gz | grep -v .sha256 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing ghz: gRPC benchmarking and load testing tool..."
