...
install_hyperfine() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/sharkdp/hyperfine/releases/latest |
  ...
  tar -xvf hyperfine*86_64-unknown-linux-gnu.tar.gz
  mv hyperfine*86_64-unknown-linux-gnu/hyperfine $DEST
