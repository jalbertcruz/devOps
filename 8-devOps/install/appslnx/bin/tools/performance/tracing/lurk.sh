...
install_lurk() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/JakWai01/lurk/releases/latest |
  ...
  echo "📥 Installing lurk: A pretty (simple) alternative to strace..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf lurk-x86_64-unknown-linux-gnu.tar.gz
