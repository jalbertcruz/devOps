...
install_lazydocker() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/jesseduffield/lazydocker/releases/latest |
  ...
  tar -xvf lazydocker_*inux_x86_64.tar.gz
  mv lazydocker $DEST
