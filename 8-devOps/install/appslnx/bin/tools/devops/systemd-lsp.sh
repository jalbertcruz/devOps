...
app_name=systemd_lsp
install_systemd_lsp() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/JFryy/systemd-lsp/releases/latest |
    grep browser_download_url | grep systemd-lsp-x86_64-unknown-linux-gnu | head -n 1 |
    cut -d '"' -f4)
