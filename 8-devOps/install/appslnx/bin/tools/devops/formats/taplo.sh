...
install_taplo() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/tamasfe/taplo/releases/latest |
    grep browser_download_url | grep inux-x86_64.gz | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing taplo: A TOML toolkit written in Rust..."
