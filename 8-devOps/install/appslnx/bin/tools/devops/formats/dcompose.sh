...
install_decompose() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/s0rg/decompose/releases/latest |
    grep browser_download_url | grep inux_x86_64.tar.gz |
    cut -d '"' -f4)
  ...
  tar -xvf decompose*inux_x86_64.tar.gz
