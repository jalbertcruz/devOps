...
install_catatonit() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/openSUSE/catatonit/releases/latest |
    grep browser_download_url | grep catatonit.x86_64 | head -n 1)
  ...
  echo "📥 Installing catatonit: A container init that is so simple it's effectively brain-dead..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  mv catatonit.x86_64 catatonit
  chmod +x catatonit
  mv catatonit $DEST
