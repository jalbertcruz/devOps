...
app_name=consul_template
install_consul_template() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  version=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/hashicorp/consul-template/releases/latest |
  ...
  mkdir -p $DEST
  wget $url
  unzip consul-template*inux_amd64.zip
  mv consul-template $DEST
