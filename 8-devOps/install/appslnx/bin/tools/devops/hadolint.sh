...
install_hadolint() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/hadolint/hadolint/releases/latest |
    grep browser_download_url | grep inux-x86_64 | grep -v beta | head -n 1 |
    cut -d '"' -f4)
