...
install_dotenvx() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/dotenvx/dotenvx/releases/latest |
  ...
  tar -xvf dotenvx*linux-amd64.tar.gz
  mv dotenvx $DEST
