...
install_direnv() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/direnv/direnv/releases/latest |
    grep browser_download_url | grep direnv.linux-amd64 |
    cut -d '"' -f4)
