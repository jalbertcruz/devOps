...
install_lnav() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/tstack/lnav/releases/latest |
    grep browser_download_url | grep inux-musl-x86_64.zip | head -n 1 |
    cut -d '"' -f4)
