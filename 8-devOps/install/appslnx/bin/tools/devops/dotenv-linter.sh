...
app_name=dotenv_linter
install_dotenv_linter() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/dotenv-linter/dotenv-linter/releases/latest |
    grep browser_download_url | grep linux-x86_64.tar.gz |
    cut -d '"' -f4)
  ...
  tar -xvf dotenv-linter-linux-x86_64.tar.gz
  mv dotenv-linter $DEST
