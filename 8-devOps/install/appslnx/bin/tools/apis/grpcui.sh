...
install_grpcui() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/fullstorydev/grpcui/releases/latest |
  ...
  tar -xvf grpcui*inux_x86_64.tar.gz
  mv grpcui $DEST
