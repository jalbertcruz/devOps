...
app_name=grpc_client_cli
install_grpc_client_cli() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/vadimi/grpc-client-cli/releases/latest |
    grep browser_download_url | grep inux_x86_64.tar.gz | head -n 1)
