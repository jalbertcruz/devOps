...
app_name=vacuum_openapi
install_vacuum_openapi() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/daveshanley/vacuum/releases/latest |
    grep browser_download_url | grep linux_x86_64.tar.gz |
    cut -d '"' -f4)
