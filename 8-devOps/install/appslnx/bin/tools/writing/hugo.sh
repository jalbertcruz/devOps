...
install_hugo() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/gohugoio/hugo/releases/latest |
    grep browser_download_url | grep linux-amd64.tar.gz | grep extended | grep -v withdeploy |
    cut -d '"' -f4)
  ...
  tar -xvf hugo_extended_*_linux-amd64.tar.gz
