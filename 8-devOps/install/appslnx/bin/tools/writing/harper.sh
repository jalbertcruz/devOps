...
install_harper_ls() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -s https://api.github.com/repos/Automattic/harper/releases/latest |
    grep browser_download_url | grep harper-ls-x86_64-unknown-linux-gnu.tar.gz | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing harper_ls..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -LO
  tar -xvf harper-ls-x86_64-unknown-linux-gnu.tar.gz
  cp harper-ls $DEST
