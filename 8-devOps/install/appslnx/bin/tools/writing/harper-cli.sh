...
install_harper_cli() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/Automattic/harper/releases/latest |
    grep browser_download_url | grep harper-cli-x86_64-unknown-linux-gnu.tar.gz | head -n 1 |
    cut -d '"' -f4)
