...
install_texlab() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/latex-lsp/texlab/releases/latest |
    grep browser_download_url | grep 86_64-linux.tar.gz |
    head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing texlab: An implementation of the Language Server Protocol for LaTeX..."
