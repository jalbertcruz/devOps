...
app_name=languagetool_rust
install_languagetool_rust() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/jeertmans/languagetool-rust/releases/latest |
  ...
  echo "📥 Installing languagetool-rust: LanguageTool API in Rust..."
