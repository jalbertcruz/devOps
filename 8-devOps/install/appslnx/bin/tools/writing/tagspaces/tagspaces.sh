...
install_tagspaces() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/tagspaces/tagspaces/releases/latest |
    grep browser_download_url | grep .tar.gz | grep tagspaces-linux-x64 | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing TagSpaces..."
  echo "Downloading $url"
  rm -Rf $DEST || true
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf tagspaces-linux-x64*.tar.gz
  rm tagspaces-linux-x64*.tar.gz
  cp -Rf tagspaces-linux-x64*/* $DEST
