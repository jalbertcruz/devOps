...
install_marksman() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/artempyanykh/marksman/releases/latest |
    grep browser_download_url | grep marksman-linux-x64 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing marksman: Write Markdown with code assist and intelligence in the comfort of your favourite editor....."
