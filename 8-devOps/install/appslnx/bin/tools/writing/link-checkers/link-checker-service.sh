...
app_name=link_checker_service
install_link_checker_service() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/siemens/link-checker-service/releases/latest |
    grep browser_download_url | grep link-checker-service_Linux_x86_64.tar.gz | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing link-checker-service..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf link-checker-service_Linux_x86_64.tar.gz
  cp link-checker-service $DEST
