...
install_lychee() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/lycheeverse/lychee/releases/latest |
    grep browser_download_url | grep lychee-x86_64-unknown-linux-gnu.tar.gz |
    cut -d '"' -f4)
  ...
  mv lychee-x86_64-unknown-linux-gnu/lychee $DEST
