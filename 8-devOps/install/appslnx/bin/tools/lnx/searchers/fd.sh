...
install_fd() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/sharkdp/fd/releases/latest |
  ...
  tar -xvf fd-*-x86_64-unknown-linux-gnu.tar.gz
  maybe_copy_fish_completions_files
  mv fd-*-x86_64-unknown-linux-gnu/fd $DEST
