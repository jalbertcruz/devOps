...
install_tv() {
  ...
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/alexpasmantier/television/releases/latest |
  ...
  tar -xvf tv*x86_64-unknown-linux-gnu.tar.gz
  maybe_copy_fish_completions_files
  maybe_copy_man_pages_files
  cp tv*x86_64-unknown-linux-gnu/tv $DEST
