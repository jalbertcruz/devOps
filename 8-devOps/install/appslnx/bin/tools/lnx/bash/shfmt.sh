...
install_shfmt() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/mvdan/sh/releases/latest |
    grep browser_download_url | grep inux_amd64 |
    cut -d '"' -f4)
