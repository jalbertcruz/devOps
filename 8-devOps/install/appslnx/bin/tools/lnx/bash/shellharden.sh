...
app_name=shellharden
install_shellharden() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/anordal/shellharden/releases/latest |
  ...
  echo "📥 Installing shellharden: The corrective bash syntax highlighter..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf shellharden-x86_64-unknown-linux-gnu.tar.gz
