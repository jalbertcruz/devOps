...
install_cheat() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/cheat/cheat/releases/latest |
    grep browser_download_url | grep inux-amd64.gz |
    cut -d '"' -f4)
