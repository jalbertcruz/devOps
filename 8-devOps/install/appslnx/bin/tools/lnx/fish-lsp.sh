...
install_fish_lsp() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/ndonfris/fish-lsp/releases/latest |
    grep browser_download_url | grep fish-lsp.standalone.with-all-assets.tar |
    cut -d '"' -f4)
  ...
  tar -xvf fish-lsp.standalone.with-all-assets.tar
  maybe_copy_fish_completions_files
  maybe_copy_man_pages_files
  mv bin/fish-lsp $DEST
