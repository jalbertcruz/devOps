...
install_zoxide() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/ajeetdsouza/zoxide/releases/latest |
  ...
  tar -xvf zoxide*x86_64-unknown-linux-musl.tar.gz
