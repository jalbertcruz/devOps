...
install_choose() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/theryangeary/choose/releases/latest |
  ...
  echo "📥 Installing choose (sed/awk alternative)..."
