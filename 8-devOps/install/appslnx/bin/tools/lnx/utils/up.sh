...
install_up() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/akavel/up/releases/latest |
    grep browser_download_url | grep up | head -n 1 |
    cut -d '"' -f4)
