...
install_bat() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/sharkdp/bat/releases/latest |
  ...
  tar -xvf bat-*-x86_64-unknown-linux-gnu.tar.gz
  maybe_copy_fish_completions_files
  mv bat-*-x86_64-unknown-linux-gnu/bat $DEST
