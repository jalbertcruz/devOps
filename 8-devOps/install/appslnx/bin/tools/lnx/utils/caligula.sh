...
install_caligula() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/ifd3f/caligula/releases/latest |
    grep browser_download_url | grep x86_64-linux | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing caligula (TUI for disk imaging)..."
