...
install_gdu() {
  ...
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/dundee/gdu/releases/latest |
    grep browser_download_url | grep linux_amd64.tgz | head -n 1 |
    cut -d '"' -f4)
