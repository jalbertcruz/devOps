...
install_dust() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/bootandy/dust/releases/latest |
  ...
  tar -xvf dust-*-x86_64-unknown-linux-gnu.tar.gz
  maybe_copy_fish_completions_files
  maybe_copy_man_pages_files
  mv dust-*-x86_64-unknown-linux-gnu/dust $DEST
