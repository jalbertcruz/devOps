...
install_jump() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/gsamokovarov/jump/releases/latest |
    grep browser_download_url | grep jump_linux_amd64_binary | head -n 1 |
    cut -d '"' -f4)
  ...
  mv jump_linux_amd64_binary jump
  chmod +x jump
  mv jump $DEST
