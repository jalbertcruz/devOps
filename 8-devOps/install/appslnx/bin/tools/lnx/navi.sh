...
install_navi() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/denisidoro/navi/releases/latest |
  ...
  tar -xvf navi*86_64-unknown-linux-musl.tar.gz
