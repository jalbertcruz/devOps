...
install_watchexec() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/watchexec/watchexec/releases/latest |
    grep browser_download_url | grep 86_64-unknown-linux-gnu.tar.xz | grep -v .b3 | grep -v .sha256 | grep -v .sha512 |
    cut -d '"' -f4)
  ...
  tar -xvf watchexec-*-x86_64-unknown-linux-gnu.tar.xz
  maybe_copy_fish_completions_files
  mv watchexec-*-x86_64-unknown-linux-gnu/watchexec $DEST
