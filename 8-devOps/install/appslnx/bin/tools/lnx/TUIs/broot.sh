...
install_broot() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/Canop/broot/releases/latest | grep browser_download_url | grep broot | grep .zip | head -n 1 |
    cut -d '"' -f4)
  ...
  mv x86_64-unknown-linux-gnu/broot $DEST
