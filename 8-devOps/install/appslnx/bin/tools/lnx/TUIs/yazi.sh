...
install_yazi() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/sxyazi/yazi/releases/latest |
    grep browser_download_url | grep 86_64-unknown-linux-gnu.zip |
    cut -d '"' -f4)
  ...
  mv yazi-x86_64-unknown-linux-gnu/ya* $DEST
