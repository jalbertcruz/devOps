...
install_ov() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/noborus/ov/releases/latest |
    grep browser_download_url | grep inux_amd64.zip | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing ov: terminal-based text viewer. It is a so-called terminal pager...."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  unzip ov*inux_amd64.zip
