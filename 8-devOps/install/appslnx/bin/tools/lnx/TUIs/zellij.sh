...
install_zellij() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/zellij-org/zellij/releases/latest |
    grep browser_download_url | grep 86_64-unknown-linux-musl.tar.gz | grep -v no-web |
    cut -d '"' -f4)
