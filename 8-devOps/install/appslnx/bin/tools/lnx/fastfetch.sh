...
install_fastfetch() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/fastfetch-cli/fastfetch/releases/latest |
  ...
  mv fastfetch-linux-amd64/usr/bin/fastfetch $DEST
  mv fastfetch-linux-amd64/usr/bin/flashfetch $DEST
