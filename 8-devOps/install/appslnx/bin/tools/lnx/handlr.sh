...
app_name=handlr_regex
install_handlr_regex() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/Anomalocaridid/handlr-regex/releases/latest |
    grep browser_download_url | grep handlr | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing handlr..."
  # handlr updates $HOME/.config/mimeapps.list
