...
install_overmind() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/DarthSim/overmind/releases/latest |
    grep browser_download_url | grep inux-amd64.gz | grep -v .sha256sum |
    cut -d '"' -f4)
  ...
  echo "📥 Installing overmind: Process manager for Procfile-based applications and tmux..."
