...
install_mprocs() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/pvolok/mprocs/releases/latest |
    grep browser_download_url | grep linux-x86_64-musl.tar.gz |
    cut -d '"' -f4)
  ...
  tar -xzf mprocs-*-linux-x86_64-musl.tar.gz
