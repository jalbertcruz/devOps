...
install_pueued() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/Nukesor/pueue/releases/latest |
    grep browser_download_url | grep pueued-x86_64-unknown-linux-musl |
    cut -d '"' -f4)
  ...
  mv pueued-x86_64-unknown-linux-musl pueued
  chmod +x pueued
  mv pueued $DEST
