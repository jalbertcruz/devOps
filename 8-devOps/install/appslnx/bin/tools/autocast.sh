...
install_autocast() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/k9withabone/autocast/releases/latest |
    grep browser_download_url | grep 86_64-unknown-linux-gnu.tar.xz | head -n 1)
