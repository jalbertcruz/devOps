...
install_sshm() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/Gu1llaum-3/sshm/releases/latest |
  ...
  tar -xvf sshm_Linux_x86_64.tar.gz
  cp sshm $DEST
