...
install_socat() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/3ndG4me/socat/releases/latest |
    grep browser_download_url | grep socatx64.bin |
    cut -d '"' -f4)
  ...
  echo "📥 Installing socat: a relay for bidirectional data transfers between two independent data channels..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  mv socatx64.bin socat
  chmod +x socat
  mv socat $DEST
