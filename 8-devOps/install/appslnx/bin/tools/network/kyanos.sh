...
install_kyanos() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/hengyoush/kyanos/releases/latest |
    grep browser_download_url | grep linux_amd64.tar.gz | head -n 1 | cut -d '"' -f4)
