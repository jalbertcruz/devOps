...
install_termshark() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/gcla/termshark/releases/latest |
    grep browser_download_url | grep linux_x64.tar.gz | head -n 1 |
    cut -d '"' -f4)
  ...
  tar -xvf termshark*linux_x64.tar.gz
  cp termshark*linux_x64/termshark $DEST
