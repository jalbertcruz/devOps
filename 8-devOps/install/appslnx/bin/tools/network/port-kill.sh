...
install_port_kill() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/treadiehq/port-kill/releases/latest |
    grep browser_download_url | grep port-kill-linux | head -n 1 |
    cut -d '"' -f4)
