...
install_tcpproxy() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/mqudsi/tcpproxy/releases/latest |
  ...
  echo "📥 Installing tcpproxy: A cross-platform TCP proxy in tokio and Rust..."
