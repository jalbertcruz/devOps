...
install_hoverfly() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/SpectoLabs/hoverfly/releases/latest |
    grep browser_download_url | grep bundle_linux_amd64.zip | head -n 1)
