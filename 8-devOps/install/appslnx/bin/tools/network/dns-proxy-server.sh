...
app_name=dns_proxy_server
install_dns_proxy_server() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/mageddo/dns-proxy-server/releases/latest |
    grep browser_download_url | grep dns-proxy-server-linux-amd64 | head -n 1 |
    cut -d '"' -f4)
