...
install_oryx() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/pythops/oryx/releases/latest |
    grep browser_download_url | grep 86_64-unknown-linux-musl | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing oryx (TUI for sniffing network traffic using eBPF)..."
