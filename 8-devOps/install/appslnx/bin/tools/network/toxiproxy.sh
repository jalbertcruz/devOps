...
install_toxiproxy() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/Shopify/toxiproxy/releases/latest |
  ...
  echo "📥 Installing toxiproxy: A TCP proxy to simulate network and system conditions for chaos and resiliency testing..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf toxiproxy*inux_amd64.tar.gz
  maybe_copy_fish_completions_files
  maybe_copy_man_pages_files
  mv toxiproxy-cli toxiproxy-server $DEST
