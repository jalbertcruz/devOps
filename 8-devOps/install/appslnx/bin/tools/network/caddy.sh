...
install_caddy() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/caddyserver/caddy/releases/latest |
  ...
  tar -xvf caddy*inux_amd64.tar.gz
