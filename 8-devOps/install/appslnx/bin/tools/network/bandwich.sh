...
install_bandwhich() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/imsnif/bandwhich/releases/latest |
  ...
  tar -xvf bandwhich*86_64-unknown-linux-gnu.tar.gz
