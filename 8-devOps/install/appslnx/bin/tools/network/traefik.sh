...
app_name=traefik
install_traefik() {
  # TODO: traefik needs a special case because it can have previous mayor versions released
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/traefik/traefik/releases/latest |
    grep browser_download_url | grep inux_amd64.tar.gz | grep -v rc | head -n 1 |
    cut -d '"' -f4)
  ...
  tar -xvf traefik*inux_amd64.tar.gz
