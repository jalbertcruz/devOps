...
app_name=go_tcp_proxy
install_go_tcp_proxy() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/jpillora/go-tcp-proxy/releases/latest |
    grep browser_download_url | grep inux_amd64.gz |
    cut -d '"' -f4)
