...
install_tun2socks() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/xjasonlyu/tun2socks/releases/latest |
    grep browser_download_url | grep linux-amd64-v3.zip | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing tun2socks (powered by gVisor TCP/IP stack)..."
  ...
  rm *.zip
  cp tun2socks-linux-amd64-v3 $DEST/tun2socks
