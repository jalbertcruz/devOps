...
install_gor() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/buger/goreplay/releases/latest |
    grep browser_download_url | grep x64.tar.gz | grep -v rc | head -n 1 |
    cut -d '"' -f4)
