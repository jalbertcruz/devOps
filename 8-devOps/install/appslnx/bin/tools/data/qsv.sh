...
install_qsv() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/dathere/qsv/releases/latest |
    grep browser_download_url | grep x86_64-unknown-linux-gnu.zip | head -n 1 |
    cut -d '"' -f4)
