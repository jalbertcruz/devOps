...
install_jid() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/simeji/jid/releases/latest |
    grep browser_download_url | grep jid_linux_amd64.zip | head -n 1 |
    cut -d '"' -f4)
