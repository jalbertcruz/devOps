...
install_jless() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/PaulJuliusMartinez/jless/releases/latest |
    grep browser_download_url | grep 86_64-unknown-linux-gnu.zip | head -n 1 |
    cut -d '"' -f4)
  ...
  unzip jless*x86_64-unknown-linux-gnu.zip
