...
install_protoc() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/protocolbuffers/protobuf/releases/latest |
    grep browser_download_url | grep inux-x86_64 |
    cut -d '"' -f4)
  ...
  unzip protoc*inux-x86_64.zip
  mv bin/protoc $DEST
  rm -Rf $DEST/../protoc/include/google || true
  mkdir -p $DEST/../protoc/include
  cp -Rf include/* $DEST/../protoc/include
