...
install_buf() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/bufbuild/buf/releases/latest |
    grep browser_download_url | grep inux-x86_64.tar.gz | head -n 1)
