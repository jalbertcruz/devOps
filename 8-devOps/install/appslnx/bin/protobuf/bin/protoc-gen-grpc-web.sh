...
app_name=protoc_gen_grpc_web
install_protoc_gen_grpc_web() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/grpc/grpc-web/releases/latest |
    grep browser_download_url | grep inux-x86_64 | grep -v sha256 |
    cut -d '"' -f4)
  ...
  mv protoc-gen-grpc-web*inux-x86_64 protoc-gen-grpc-web
  chmod +x protoc-gen-grpc-web
  mv protoc-gen-grpc-web $DEST
