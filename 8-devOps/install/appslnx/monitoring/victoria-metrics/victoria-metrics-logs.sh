...
app_name=victoria_metrics_logs
install_victoria_metrics_logs() {
  ...
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/VictoriaMetrics/VictoriaLogs/releases/latest |
    grep browser_download_url | grep .tar.gz | grep amd64 | grep linux | grep victoria-logs | grep -v enterprise | head -n 1 |
    cut -d '"' -f4)
  ...
  echo "📥 Installing victoria-metrics logs..."
  ...
  cp victoria-logs-prod $DEST/victoria-logs
