...
install_victoria_metrics() {
  ...
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/VictoriaMetrics/VictoriaMetrics/releases/latest |
    grep browser_download_url | grep .tar.gz | grep amd64 | grep linux | grep victoria-metrics | grep -v enterprise | grep -v cluster | head -n 1 |
    cut -d '"' -f4)
  ...
  tar -xvf victoria-metrics-linux-amd64*.tar.gz
  cp victoria-metrics-prod $DEST/victoria-metrics
