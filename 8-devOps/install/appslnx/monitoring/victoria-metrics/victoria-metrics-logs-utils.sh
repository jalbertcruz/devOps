...
app_name=victoria_metrics_logs_utils
install_victoria_metrics_logs_utils() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/VictoriaMetrics/VictoriaLogs/releases/latest |
    grep browser_download_url | grep .tar.gz | grep amd64 | grep linux | grep vlutils | grep -v enterprise | head -n 1 |
    cut -d '"' -f4)
