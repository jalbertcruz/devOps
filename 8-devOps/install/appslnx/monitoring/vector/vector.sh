...
install_vector() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/vectordotdev/vector/releases/latest |
  ...
  echo "📥 Installing vector: A high-performance observability data pipeline...."
  echo "Downloading $url"
  rm -Rf $DEST || true
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf vector*86_64-unknown-linux-gnu.tar.gz
  maybe_copy_fish_completions_files
  cp -Rf vector-x86_64-unknown-linux-gnu/* $DEST
