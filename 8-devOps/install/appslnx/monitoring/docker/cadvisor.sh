...
install_cadvisor() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/google/cadvisor/releases/latest |
    grep browser_download_url | grep linux-amd64 | head -n 1 |
    cut -d '"' -f4)
  ...
  mv cadvisor*linux-amd64 cadvisor
  chmod +x cadvisor
  mv cadvisor $DEST
