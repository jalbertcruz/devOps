...
install_sourcegit() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -s https://api.github.com/repos/sourcegit-scm/sourcegit/releases/latest |
    grep browser_download_url | grep linux.amd64.AppImage | head -n 1)
  ...
  mv sourcegit*.linux.amd64.AppImage a.AppImage
  chmod +x a.AppImage
