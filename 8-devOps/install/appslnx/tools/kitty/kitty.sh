...
install_kitty() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/kovidgoyal/kitty/releases/latest |
    grep browser_download_url | grep x86_64.txz | head -n 1 |
    cut -d '"' -f4)
