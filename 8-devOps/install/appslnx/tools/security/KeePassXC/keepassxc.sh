...
install_keepassxc() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -s https://api.github.com/repos/keepassxreboot/keepassxc/releases/latest |
    grep browser_download_url | grep x86_64.AppImage | grep -v DIGEST | grep -v sig | grep -v zsync | head -n 1)
  ...
  mv KeePassXC*x86_64.AppImage a.AppImage
  chmod +x a.AppImage
