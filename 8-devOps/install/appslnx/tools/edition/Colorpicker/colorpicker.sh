...
install_colorpicker() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/Toinane/colorpicker/releases/latest |
  ...
  mv Colorpicker*AppImage a.AppImage
  chmod +x a.AppImage
