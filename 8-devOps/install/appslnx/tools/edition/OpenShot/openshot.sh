...
install_OpenShot() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/OpenShot/openshot-qt/releases/latest |
    grep browser_download_url | grep AppImage |
    grep -v .sha256sum |
    grep -v .sha256sum.verify |
    grep -v torrent |
    head -n 1)
  ...
  mv OpenShot*AppImage a.AppImage
  chmod +x a.AppImage
