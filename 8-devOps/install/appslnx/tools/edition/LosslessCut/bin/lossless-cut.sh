...
app_name=lossless_cut
install_lossless_cut() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/mifi/lossless-cut/releases/latest |
    grep browser_download_url | grep inux-x64.tar.bz2 | head -n 1)
  ...
  echo "📥 Installing lossless-cut..."
  echo "Downloading $url"
  mkdir -p $DEST
  echo -n $url | xargs curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -LO
  tar -xvf LosslessCut*.tar.bz2
  rm -rf LosslessCut*.tar.bz2
  cp -Rf LosslessCut*/* $DEST
