...
app_name=podman_desktop
install_podman_desktop() {
  mkdir -p "$TMP_DIR/$app_name"
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/podman-desktop/podman-desktop/releases/latest |
    grep browser_download_url | grep .tar.gz | head -n 1 |
    cut -d '"' -f4)
