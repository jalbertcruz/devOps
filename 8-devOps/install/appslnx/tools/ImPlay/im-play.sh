...
install_im_play() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/tsl0922/ImPlay/releases/latest |
    grep browser_download_url | grep inux.AppImage | head -n 1)
