...
install_sioyek() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/ahrm/sioyek/releases/latest |
    grep browser_download_url | grep sioyek-release-linux.zip | head -n 1)
  ...
  mv Sioyek*.AppImage sioyek.AppImage
  chmod +x sioyek.AppImage
  rm -rf $DEST/squashfs-root || true
  ./sioyek.AppImage --appimage-extract
