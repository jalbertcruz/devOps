...
install_krakend() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/krakend/krakend-ce/releases/latest |
    grep browser_download_url | grep amd64_generic-linux.tar.gz | head -n 1)
  ...
  tar -xvf krakend*amd64_generic-linux.tar.gz
  maybe_copy_fish_completions_files
  mv usr/bin/krakend $DEST
