...
install_httptoolkit() {
  ...
  cd "$TMP_DIR/$app_name"
  url1=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/httptoolkit/httptoolkit-desktop/releases/latest |
    grep browser_download_url | grep x64.AppImage | head -n 1)
  ...
  mv HttpToolkit*x64.AppImage HttpToolkit.AppImage
  chmod +x HttpToolkit.AppImage
  ./HttpToolkit.AppImage --appimage-extract
