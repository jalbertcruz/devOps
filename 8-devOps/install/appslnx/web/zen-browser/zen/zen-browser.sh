...
install_zen_browser() {
  ...
  cd "$TMP_DIR/$app_name"
  url=$(curl -H "Authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" -s https://api.github.com/repos/zen-browser/desktop/releases/latest | grep browser_download_url | grep linux-x86_64.tar.xz | head -n 1 |
    cut -d '"' -f4)
